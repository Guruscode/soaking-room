    ...
    <div className="rounded-xl border border-slate-200 p-4">
      ...
      <div className="mt-4 space-y-4">
        {notifications.map((item) => {
          const meetingUrl = normalizeExternalUrl(item.meetingLink)
            ...
            <article key={item.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              {item.classStartAt ? (
                <div className="mt-3 rounded-lg border border-slate-200 bg-white p-3 text-sm text-slate-700">
                  ...
                  {item.classMode === "online" && meetingUrl ? (
                    <Link href={meetingUrl} className="mt-2 inline-block text-sm font-medium text-slate-900 underline" target="_blank" rel="noopener noreferrer">
                      Open class link
                    </Link>
                  ) : null}
